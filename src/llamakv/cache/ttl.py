...
class TTLCache(CacheStrategy):
    ...
    def _enforce_size_limit(self) -> int:
        ...
        items.sort(
            key=lambda x: x[1].expiry if x[1].expiry is not None else float("inf")
        )
    ...
    def cleanup(self) -> int:
        ...
        with self._lock:
            now = time.time()
            expired_keys = [
                key for key, value in self._cache.items() if value.is_expired()
            ]

            # Remove expired keys
            for key in expired_keys:
                if self._max_size_bytes is not None:
                    self._estimated_size_bytes -= self._estimate_item_size(
                        key, self._cache[key]
                    )
                del self._cache[key]
    ...
    def set(self, key: Key, value: Value) -> None:
        ...
        with self._lock:
            ...
            if len(self._cache) >= self._capacity and key not in self._cache:
                ...
                items.sort(
                    key=lambda x: (
                        x[1].expiry if x[1].expiry is not None else float("inf")
                    )
                )
                ...
                stats.update(
                    {
                        ...
                        "size_utilization": (
                            self._estimated_size_bytes / self._max_size_bytes
                        )
                        * 100,
                    }
                )
